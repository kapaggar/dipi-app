...
**Now shipping:** Vertical 2 desk **1.24.1** (`versionCode` 38) on `main` — `feat/vertical-1` is merged and gone. Unmerged `feat/desk-gap` carries **1.19.0** (centre tiles, letters, applicant history, desk search — reads only, plus QA screenshots in `docs/qa-1.19.0/`). Default host is live `https://dipi.vridhamma.org`.
