...
**Shipped:** Vertical 2 desk on `feat/ui-gap-closure`, **1.30.3** (`versionCode` 49). `feat/vertical-1` is merged and deleted. The **Day-11 export** (`SheetExport.Day11Report` → `GET /report-day11/{cid}/{courseId}` PDF) is a solid full-width fourth-line Board chip under the 3×4 shelves (T1, 2026-08-31); phone hub overflow still reaches it via `hubSheetLabel`.
...
13. **Board sheet exports (12 shelves + Day-11) are served by the live desk:** streamed PDF `GET /course-pdf-m|f/{cid}/{courseId}`; streamed Excel `GET /laundry-list|valuable-list/{cid}/{courseId}`; print HTML `GET /day0-list|teacher-list|manager-list|student-chit|checking-slip|seating/{cid}/{courseId}`; Day 0 summary = the `#day-summary` block of `GET /zero-day/{cid}/{courseId}`; course report = its own Drupal form POST (CSV); Day-11 course summary = streamed PDF `GET /report-day11/{cid}/{courseId}` as a fourth-line chip, not a 13th cell in the 3×4 grid. Sheets are display-only — in-memory / `cacheDir/sheets` only, wiped on logout/session-expiry/erase-all. **NEVER send an `r` query param on sheet GETs** — its mere presence triggers server-side bulk seat auto-allocation. App edit page `GET /app/{id}/edit` is rendered display-only.
...
17. **Status vocabulary (T3):** the worklist page's own `edit-app-status` select, with roster fallback. Display and send strings only; never invent statuses in Kotlin.
