...
**Shipped:** Vertical 2 desk + **Course ops (assistant-teacher mode)** + **Sheets v5** + **Reach v5 UI** on `feat/reach-v5-ui`, **1.36.1** (`versionCode` 59). Board seating is the Course ops hall (native 5h), not `GET /seating` HTML. The **Day-11 export** stays a full-width fourth-line Board chip; **Course report left the Board** (v5 T4) for its native centre-dashboard surface (v5 T3). Course ops: teacher-at-the-bottom seating (r2), chowky/chair rail, worklist + application buffering on entry with a pull progress strip.
