...
**Shipped:** Vertical 2 desk on `main`, **1.30.5** (`versionCode` 51). Every feature branch is merged and deleted. The **Day-11 export** (`SheetExport.Day11Report` → `GET /report-day11/{cid}/{courseId}` PDF) is a solid full-width fourth-line Board chip under the 3×4 shelves (T1, 2026-08-31); phone hub overflow still reaches it via `hubSheetLabel`.
