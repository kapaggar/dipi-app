...
**Current release 1.45.3** (`versionCode` 91): Course ops student card reads Course History `Teacher(s)` under First / Most Recent course (edit-form teacher inputs only when the view page omits them); larger `OLD · OM5` chip; A-List hidden. Prior **1.45.2** (`versionCode` 90): Course ops seating/card polish — age on seat cells and a lighter Teacher-list AGE column; student card Room/Seat beside the photo; Gender/Nationality/Monk/Applied On hidden; first/last course teacher from Course History when present; elevated dest chips and card ‹ ›; system Back pops the card to the seating plan; backrest is a dark top bar, not the faint `⌐` glyph. Course ops stays read-only. Prior **1.45.1** (`versionCode` 89): corrected photos replay `GET`+`POST /app/{id}/edit`. See `docs/plans/2026-09-07-photo-correction-track-a.md`.
