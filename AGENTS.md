...
**Current release 2.2.0** (`versionCode` 102): The desk rail starts at 840dp, compact detail has Back to list, the Board shows count derivations and last-day reconciliation, IDs are masked until Reveal, and desk controls use 14sp/48dp minimums. Active confirmation-number holders remain on the roll except Cancelled/Left/Rejected/Regret/Duplicate; WaitList remains held on the roll. See `docs/plans/2026-09-13-desk-2.2.md`. Prior **2.1.1** (`versionCode` 101): Desk roll drops Left (same as Cancelled/Duplicate/Rejected); a Female · New chip on the desk top bar names the persisted gender/seniority scope and tap clears it. Prior **2.1.0** (`versionCode` 100): Phone Day 0 summary uses the Board sheet (or Expected+Confirmed on finalized); Zero Day drops Cancelled/Duplicate/Left from arrivals; Applications empty copy names the filters; Centre Settings refreshes rooms on open; Course ops portrait stacks the student card and uses two-line teacher rows; Centre heroes the running older course; Course ops mode switch lands on Teacher list; day chip and Last synced read the name window / relative age. Prior **2.0.1** (`versionCode` 99): Theme rendering uses the effective Steel night palette in Dark while retaining the saved Light skin, with shared surface roles for Settings, course, applicant and desk panes; no transport or persistence changes. Prior **2.0.0** (`versionCode` 98): Fetch parsing and observed-roll audits run off Main; hot parser regexes are reused; the minute clock stops in background/screen-off; finalized Day 0 summary skips Zero Day. Keep-alive and fetch order unchanged. See `docs/plans/2026-09-10-desk-performance.md`. Prior **1.46.2** (`versionCode` 96): Photo review, correction, live update and ML Kit scan default off; ordinary applicant photo display remains. Build `-Pdipi.photoReview=true` for the enabled APK. See `docs/PHOTO-BUILDS.md`. Prior **1.46.1** (`versionCode` 95): Room Chart age is the muted 12sp number at the reserved top-right only (lower-right age removed). Prior **1.46.0** (`versionCode` 94): Course report shows teacher names from ConductingTeachers / AssistingTeachers; Audit ports Aadhaar-length and related in-memory ID checks (no raw NPI in Room); Room Chart compact empty rows and age at top-right as well as bottom-right; Settings course-ops Health answers row is one clean sentence. Prior **1.45.5** (`versionCode` 93): Room Chart age is the number only, bottom-right; Old is a solid accent border, New is the same stroke short-dashed; Old / New / Available legend sits above the grid. Prior **1.45.4** (`versionCode` 92): Settings → Simulate offline applies on tap. Prior **1.45.3** (`versionCode` 91): Course ops student card reads Course History `Teacher(s)` under First / Most Recent course (edit-form teacher inputs only when the view page omits them); larger `OLD · OM5` chip; A-List hidden. Prior **1.45.2** (`versionCode` 90): Course ops seating/card polish — age on seat cells and a lighter Teacher-list AGE column; student card Room/Seat beside the photo; Gender/Nationality/Monk/Applied On hidden; first/last course teacher from Course History when present; elevated dest chips and card ‹ ›; system Back pops the card to the seating plan; backrest is a dark top bar, not the faint `⌐` glyph. Course ops stays read-only. Prior **1.45.1** (`versionCode` 89): corrected photos replay `GET`+`POST /app/{id}/edit`. See `docs/plans/2026-09-07-photo-correction-track-a.md`.

**Prior release 1.45.0** (`versionCode` 88): Native photo correction was local-only. Staff can rotate, crop, scan conservative on-device suggestions, approve reviews and export JPEGs/ZIPs through the document picker. Drafts are encrypted and scoped by server + centre + course + source hash.

**Prior release 1.44.0** (`versionCode` 87): Native sign-in waits for restoration/auth cleanup and prevents duplicate attempts. Room Chart replaces Rooms & seats, adding age and OLD/NEW cues. Finalized courses use the existing worklist's `finalized`, `section`, and `acc` fields; Attended rooms are historical, Left never occupies a room, and allocation editing/sync is disabled. Zero Day is skipped for finalized courses. See `docs/plans/2026-09-06-login-finalized-room-chart.md`.

**Prior release 1.43.5** (`versionCode` 86): Applications **Edit on desk site ↗** opens the system browser through `/user/login?destination=app%2F{id}%2Fedit`. Drupal handles both fresh sign-in and the existing browser session. Never copy app cookies/credentials to the browser. The app refreshes the same course once after return, retaining selection and recomputing audit. The old display-only form viewer is no longer an Edit destination. Backend unchanged. See `docs/plans/2026-09-06-application-edit-session.md`.

**WhatsApp automation 1.43.4** (`versionCode` 85): Android-only, opt-in per server/centre. One separately distributed `DIPI-WA1` provisioning code replaces the pilot's two fields; no secret is bundled in the APK. Calling supports filtered recipient selection, active managed-letter preview, explicit duplicate-number review, a package-restricted accessibility sender and encrypted paused recovery. Pixel C self-tests verify exact composer text and a new outgoing message; a WhatsApp version change requires another test. Saved-name-only chats stop for manual handling. Setup now has a readiness checklist and collapsible configuration; Calling remembers the letter behind Change letter, separates batch progress from preparation, and returns to DIPI after a run. Long formatted messages are verified after expanding the new outgoing row’s Read more control; full displayed text must match, with paired bold markers accounted for and URLs unchanged. Batch progress is encrypted per server/centre/course, with migration of the legacy centre slot; another course never blocks a new batch. Discard is visible above the current batch list. App-owned descriptions are shorter and use hyphens. Automation ships off. No Drupal edits or student test messages. See `docs/plans/2026-09-06-whatsapp-automation.md`. Includes the approved 1.42.1 landscape seating-print fixes already on the tablet.
