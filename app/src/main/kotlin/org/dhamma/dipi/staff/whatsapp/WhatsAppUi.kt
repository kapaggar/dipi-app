...
private fun AutomationSettings(controller: WhatsAppController, state: WhatsAppUi) {
    ...
    Text("Keep the tablet unlocked while sending.")
    ...
    if (setupExpanded) {
        ...
        Text(if (state.configured) "Code saved. Enter a new code to replace it." else "Paste your centre’s provisioning code.")
        ...
        Text("Turn on DIPI WhatsApp automation in Android settings.")
        ...
        Text(if (tested) "Passed for WhatsApp ${profile.testedVersion}. Test again after WhatsApp updates." else "Sends a test to your own chat.")
        ...
        Text("Experimental: WhatsApp updates may break sending; account restrictions are possible. Share the secret code only with authorised admins.", style = MaterialTheme.typography.bodySmall)
        TextButton(onClick = { removal = true }, enabled = !state.busy && !state.running) { Text("Remove this tablet’s setup") }
    }
    if (removal) AlertDialog(onDismissRequest = { removal = false }, title = { Text("Remove automation settings?") }, text = { Text("Removes this centre’s code, settings and all saved batches.") },
...
private fun AutomationBatch(controller: WhatsAppController, state: WhatsAppUi) {
    ...
    val profile = state.profile ?: return
    if (!controller.ready()) {
        Text("Complete WhatsApp setup to send.")
    ...
    if (batch != null) {
        ...
        Text("Submission is not delivery confirmation. Check unknown sends in WhatsApp before continuing.")
        if (batchComplete(batch)) {
            Button(onClick = controller::discard) { Text("Done") }
            ...
            OutlinedButton(onClick = { controller.pause(); discard = true }) { Text("Discard batch") }
        }
        ...
        if (discard) AlertDialog(onDismissRequest = { discard = false }, title = { Text("Discard this batch?") }, text = { Text("Deletes this course’s saved progress. Sent messages remain; a new batch may duplicate them.") },
    ...
    Text("Chats showing only a saved name require manual sending.")
    ...
    if (controller.duplicates()) Row {
        ...
        Text("Shared numbers: send one personalised message per selected applicant.")
    }
    HorizontalDivider()
    ...
    Text("Preview may create missing applicant login details.", style = MaterialTheme.typography.bodySmall)
    state.preview?.let { preview ->
        HorizontalDivider()
        Text("3 · Review and send", style = MaterialTheme.typography.titleLarge)
        Text("Sample for ${state.candidates.firstOrNull { it.id.value == preview.applicantId }?.displayName ?: "application ${preview.applicantId}"}.")
        Text(preview.text)
