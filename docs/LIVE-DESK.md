...
in three parts: the captured transport (HAR), the PHP page inventory, and the
server facts mined from `dipi-web` (`dh_manageapp`). Backend PHP is immutable —
no `/staff/*`, no changes to `dipi-web` (AGENTS.md).
